...
@section('content')
    <form method="POST" action="{{ route('updateStudyItem', $item) }}">
        @csrf
        @method('put')
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6">
                <select id="student" class="@error('student') is-invalid @enderror form-control" disabled data-live-search="true"
                    name="student">
                    @foreach ($students as $student)
                        <option @if($item->student->id == $student->id) selected @endif value="{{ $student->id }}">{{ $student->full_name }}</option>
                    @endforeach
                </select>
                @error('student')
            ...
            <div class="col-md-6">
                <select id="group" class="@error('group') is-invalid @enderror form-control" data-live-search="true"
                    ...
                    @foreach ($groups as $group)
                        <option @if($item->group->id == $group->id) selected @endif value="{{ $group->id }}">{{ $group->name }}</option>
                    @endforeach
                </select>
                @error('group')
                    <span class="invalid-feedback" role="alert">
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6">
                <input id="date_start_study" type="date"
                    class="form-control @error('date_start_study') is-invalid @enderror" name="date_start_study" value="{{ $item->date_start_study }}">
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-6">
                <input id="date_finish_study" type="date"
                    class="form-control @error('date_finish_study') is-invalid @enderror" name="date_finish_study" value="{{ $item->date_finish_study }}">
            ...
            <div class="col-md-6">
                <select id="courses" class="@error('courses') is-invalid @enderror form-control" data-live-search="true"
                    ...
                    @foreach ($courses as $course)
                        <option @if($item->course->id == $course->id) selected @endif value="{{ $course->id }}">{{ $course->name }}</option>
                    @endforeach
                </select>
                @error('course')
                    <span class="invalid-feedback" role="alert">
